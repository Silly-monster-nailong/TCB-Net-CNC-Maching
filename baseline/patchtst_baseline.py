...
- 结果保存：输出目录下的 results.json 和 patchtst_model.pth
"""
...
class PatchTST(nn.Module):
    def __init__(self, seq_len=256, n_channels=3, patch_len=16, stride=8, d_model=32, n_heads=4, n_layers=2, dropout=0.1):
        super().__init__()
...
def train_and_evaluate(data_dir, output_dir, batch_size=256, lr=0.0005, epochs=30):
    ...
    model = PatchTST(seq_len=seq_len).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(output_dir, 'patchtst_model.pth'))
    print(f"Test Accuracy: {accuracy:.4f}, F1: {f1:.4f}, AUC: {auc:.4f}")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', required=True)
    parser.add_argument('--output_dir', default='patchtst_baseline')
