...
- 训练细节：AdamW 优化器，Cosine Annealing 学习率调度，早停机制
- 评估指标：准确率、精确率、召回率、F1 分数、AUC，以及混淆矩阵
- 结果保存：输出目录下的 results.json 和 itransformer_model.pth
"""
...
class iTransformer(nn.Module):
    def __init__(self, seq_len=256, n_features=3, d_model=96, n_heads=8, n_layers=3, d_ff=256, dropout=0.1):
        super().__init__()
...
def train_and_evaluate(data_dir, output_dir, batch_size=256, lr=0.0005, epochs=30):
    ...
    model = iTransformer(seq_len=seq_len).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(output_dir, 'itransformer_model.pth'))
    print(f"Test Accuracy: {accuracy:.4f}, F1: {f1:.4f}, AUC: {auc:.4f}")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', required=True)
    parser.add_argument('--output_dir', default='itransformer_baseline')
